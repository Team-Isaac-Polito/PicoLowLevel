...
# Board configuration
BOARD_FQBN ?= rp2040:rp2040:rpipico:flash=2097152_65536
...
compile: clean_all
	...
	@arduino-cli compile --fqbn $(BOARD_FQBN) --build-path $(BUILD_DIR) $(SKETCH_PATH) --output-dir $(OUTPUT_DIR) --libraries $(LIBS_DIR) \
		--build-property "compiler.cpp.extra_flags=$(INCLUDE_FLAGS) -D$(MODULE_DEFINE)" && \
...
compile_all:
	...
	$(MAKE) compile BUILD_DIR=$(CURDIR)/build3 OUTPUT_DIR=$(CURDIR)/out_MK2_MOD3 MODULE_DEFINE="MK2_MOD3"
